...
public class dislike extends HttpServlet {
    ...
    for (int i = 0; i < cookies.length; i++){
        ...
        try {
            status=dislikeProduct(access_token,product_id);
        } catch (IOException_Exception ex) {
            Logger.getLogger(dislike.class.getName()).log(Level.SEVERE, null, ex);
    ...
    private int dislikeProduct(java.lang.String accessToken, java.lang.String productId) throws IOException_Exception, ParseException_Exception {
        // Note that the injected javax.xml.ws.Service reference as well as port objects are not thread safe.
        // If the calling of port operations may lead to race condition some synchronization is required.
        CatalogWS.CatalogWS port = service.getCatalogWSPort();
        return port.dislikeProduct(accessToken, productId);
    }
